...
@api_view(["POST", ])
def re_send_code_api(request):
    call_response = CheckAuthCall(request).is_authenticated()
    if call_response[0] == status.HTTP_200_OK or call_response[0] == status.HTTP_409_CONFLICT:
        call_response = SignUpService(request).resend_code()
...
def verify_email_api(request):
    call_response = CheckAuthCall(request).is_authenticated()
    if call_response[0] == status.HTTP_200_OK or call_response[0] == status.HTTP_409_CONFLICT:
        call_response = SignUpService(request).verify_email()
